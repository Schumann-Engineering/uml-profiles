...
var RowSorterComponent = class {
  constructor(parentFieldset, fieldContainer, name, direction, priority, customOrder) {
    this.parentFieldset = parentFieldset;
    ...
    this.customOrderBtn.onClick(() => {
      ...
      const field = (_a = plugin.fieldIndex.fileClassesFields.get(fileClass.name)) == null ? void 0 : _a.find((f) => f.isRoot() && f.name === this.name);
...
var FilterComponent = class {
  constructor(container, name, parentFieldSet, debounced) {
    this.container = container;
    this.name = name;
    this.parentFieldSet = parentFieldSet;
...
var FieldComponent = class {
  constructor(container, parentFieldSet, name, label, columnPosition, rowPriority, isColumnHidden = false, rowSortingDirection = void 0, query = "", customOrder = []) {
    this.container = container;
    this.parentFieldSet = parentFieldSet;
    this.name = name;
    this.label = label;
  ...
  buildRowSorterComponent(fieldHeader) {
...
var FieldSet4 = class {
  ...
  getParams() {
    ...
    const sorters = Object.entries(this.rowSorters).filter(([n, s]) => {
      var _a;
  ...
  changeView(_name) {
    ...
    if (_name && savedViews.find((view) => view.name === _name)) {
      ...
      Object.keys(this.filters).forEach((name) => {
        ...
        const savedFilter = savedView == null ? void 0 : savedView.filters.find((f) => f.name === name);
      ...
      Object.keys(this.rowSorters).forEach((name) => {
        ...
        const savedSorter = (_a = savedView == null ? void 0 : savedView.sorters) == null ? void 0 : _a.find((f) => f.name === name);
        if (savedSorter) {
...
var FileClassDataviewTable = class {
  constructor(plugin, viewConfiguration, view, fileClass, maxRow, sliceStart = 0, ctx) {
    this.plugin = plugin;
...
`;
  ...
  buildDvJSRendering() {
    const fields = this.viewConfiguration.columns.filter((f) => {
      var _a;
      return !((_a = this.viewConfiguration.columns.find((_f) => _f.name === f.name)) == null ? void 0 : _a.hidden);
...
var FileClassTableView = class {
  constructor(plugin, viewContainer, tableId, fileClass, selectedView) {
    this.viewContainer = viewContainer;
  ...
  */
  buildLimitManager(container) {
    ...
    const limitInput = new import_obsidian23.TextComponent(container);
    limitInput.setValue(`${this.limit}`);
    const debounced = (0, import_obsidian23.debounce)((fieldset) => fieldset.tableView.update(this.limit), 1e3, true);
  ...
  */
  ...
  buildViewSelector() {
    ...
    const options2 = this.fileClass.getFileClassOptions();
    const savedViews = options2.savedViews || [];
    this.viewSelect = new import_obsidian23.DropdownComponent(this.viewSelectContainer);
  ...
  buildSavedViewRemoveButton(container) {
    ...
    this.viewRemoveBtn = new import_obsidian23.ButtonComponent(btnContainer).setIcon("trash").setClass("remove-button").setDisabled(!this.selectedView).setTooltip(`Remove ${this.selectedView} view from the saved views`).onClick(async () => {
      var _a;
      const options2 = this.fileClass.getFileClassOptions();
  ...
  buildFavoriteViewManager(container) {
    const btnContainer = container.createDiv({ cls: "cell" });
    this.favoriteBtn = new import_obsidian23.ButtonComponent(btnContainer);
  ...
  buildCleanFields(container) {
    const btnContainer = container.createDiv({ cls: "cell" });
    const cleanFilterBtn = new import_obsidian23.ButtonComponent(btnContainer);
  ...
  buildSaveView(container) {
    const btnContainer = container.createDiv({ cls: "cell" });
    this.saveViewBtn = new import_obsidian23.ButtonComponent(btnContainer).setIcon("save").setTooltip("Save current view (filters and sorters)").onClick(() => new CreateSavedViewModal(this.plugin, this).open());
  }
  buildHideFilters(container) {
    const btnContainer = container.createDiv({ cls: "cell" });
    const hideFilterBtn = new import_obsidian23.ButtonComponent(btnContainer);
...
var FileClassView = class extends import_obsidian24.ItemView {
...
var FileClassChoiceModal = class extends import_obsidian25.SuggestModal {
...
// src/components/FileClassViewManager.ts
var FileClassViewManager = class extends import_obsidian26.Component {
  ...
  onunload() {
    ...
    this.plugin._children.filter((child) => child.hasOwnProperty("script") && child.containerEl.getAttr("id") === this.tableId).forEach((child) => {
...
var FileClassOptionsList = class {
  ...
  createExtraOptionList(openAfterCreate = true) {
    ...
    let addMissingFieldsAction = () => {
      new import_obsidian27.Notice("Something went wrong, please check your fileClass definitions");
    };
    if (this.fromFile) {
      const dvApi = (_a = this.plugin.app.plugins.plugins.dataview) == null ? void 0 : _a.api;
      if (dvApi) {
...
var OptionsList = class {
  ...
  createAndOpenFieldModal(node) {
    ...
    if (managedField) {
      ...
      new import_obsidian28.Notice("No field with definition at this position", 2e3);
    }
  }
  ...
  buildFieldOptionsForMenu() {
    if (isMenu2(this.location)) {
      ...
      const view = this.plugin.app.workspace.getActiveViewOfType(import_obsidian28.MarkdownView);
      if (view == null ? void 0 : view.editor) {
        const action = async () => {
          if (!view.file || !(view.file instanceof import_obsidian28.TFile))
            return;
  ...
  addFieldAtCurrentPositionOption() {
    var _a;
    const currentView = this.plugin.app.workspace.getActiveViewOfType(import_obsidian28.MarkdownView);
...
var ObjectField = class extends FieldManager2 {
  ...
  createDvField(dv, p, fieldContainer, attrs) {
    ...
    (0, import_obsidian29.setIcon)(editBtn, FieldIcon[this.field.type]);
    editBtn.onclick = async () => {
      const file = this.plugin.app.vault.getAbstractFileByPath(p["file"]["path"]);
      const _eF = file instanceof import_obsidian29.TFile && await Note.getExistingFieldForIndexedPath(this.plugin, file, this.field.id);
      if (_eF)
...
var BaseSuggestModal = class extends import_obsidian30.SuggestModal {
  ...
  buildBackButton(container) {
    const backButton = new import_obsidian30.ButtonComponent(container);
...
// src/modals/BaseModal.ts
var BaseModal = class extends import_obsidian31.Modal {
  ...
  buildSimpleSaveBtn(fieldContainer) {
    ...
    infoContainer.setText("Alt+Enter to save");
    const saveBtn = new import_obsidian31.ButtonComponent(fieldContainer);
  ...
  buildFooterBtn() {
    ...
    infoContainer.setText("Alt+Enter to save");
    const confirmButton = new import_obsidian31.ButtonComponent(buttonContainer);
    confirmButton.setIcon("checkmark");
    confirmButton.onClick(async () => {
      await this.save();
      this.close();
    });
    const cancelButton = new import_obsidian31.ButtonComponent(buttonContainer);
    cancelButton.setIcon("cross");
...
in <${this.field.name}> ${this.field.fileClassName ? this.field.fileClassName : "Metadata Menu"} settings`;
              if (errorName === "SyntaxError")
                new import_obsidian32.Notice(notice, 5e3);
            }
          } else {
  ...
  buildTemplateInputItem(fieldContainer, name) {
    ...
    const input = new import_obsidian32.TextComponent(fieldContainer);
    input.inputEl.addClass("with-label");
    input.inputEl.addClass("full-width");
  ...
  buildTemplateSelectItem(fieldContainer, name, options2) {
    ...
    fieldContainer.createDiv({ cls: "spacer" });
    const selectEl = new import_obsidian32.DropdownComponent(fieldContainer);
  ...
  buildResultPreview(fieldContainer) {
    this.renderedValue = new import_obsidian32.TextAreaComponent(fieldContainer);
  ...
  buildInputEl(container) {
    const inputEl = new import_obsidian32.TextAreaComponent(container);
...
var InputField = class extends FieldManager2 {
  ...
  createSettingContainer(container, plugin) {
    ...
    const templateValue = new import_obsidian33.TextAreaComponent(templateContainer);
  ...
  createDvField(dv, p, fieldContainer, attrs = {}) {
    ...
    if ((_a = attrs.options) == null ? void 0 : _a.alwaysOn)
      spacer.hide();
    (0, import_obsidian33.setIcon)(editBtn, FieldIcon["Input" /* Input */]);
    ...
    (0, import_obsidian33.setIcon)(validateIcon, "checkmark");
    ...
    (0, import_obsidian33.setIcon)(cancelIcon, "cross");
...
var BooleanModal = class extends BaseModal {
  ...
  buildToggleEl() {
    ...
    const trueButton = new import_obsidian34.ButtonComponent(choicesContainer);
    trueButton.setButtonText("True");
    trueButton.setClass("left");
    choicesContainer.createDiv({ cls: "spacer" });
    const falseButton = new import_obsidian34.ButtonComponent(choicesContainer);
...
var NumberModal = class extends BaseModal {
  ...
  buildInputEl() {
    ...
    this.numberInput = new import_obsidian35.TextComponent(fieldContainer);
    const numberInput = this.numberInput;
    numberInput.inputEl.focus();
    numberInput.setValue(`${this.value || ""}`);
    const minusBtn = new import_obsidian35.ButtonComponent(fieldContainer);
    minusBtn.setButtonText(`- ${!!step ? step : 1}`);
    minusBtn.setDisabled(!this.fieldManager.canDecrement(numberInput.getValue()));
    const plusBtn = new import_obsidian35.ButtonComponent(fieldContainer);
...
var NumberField = class extends FieldManager2 {
  ...
  createNumberContainer(container) {
    ...
    this.numberStepValue = new import_obsidian36.TextComponent(numberStepValueContainer);
    ...
    this.numberMinValue = new import_obsidian36.TextComponent(numberMinValueContainer);
    ...
    this.numberMaxValue = new import_obsidian36.TextComponent(numberMaxValueContainer);
  ...
  createDvField(dv, p, fieldContainer, attrs = {}) {
    ...
    (0, import_obsidian36.setIcon)(editButton, FieldIcon["Number" /* Number */]);
    const decrementButton = fieldContainer.createEl("button");
    (0, import_obsidian36.setIcon)(decrementButton, "left-arrow");
    const incrementButton = fieldContainer.createEl("button");
    (0, import_obsidian36.setIcon)(incrementButton, "right-arrow");
    if (!((_a = attrs == null ? void 0 : attrs.options) == null ? void 0 : _a.alwaysOn)) {
      editButton.hide();
    ...
    (0, import_obsidian36.setIcon)(validateIcon, "checkmark");
    ...
    (0, import_obsidian36.setIcon)(cancelIcon, "cross");
    cancelIcon.onclick = (e) => {
      fieldContainer.removeChild(inputContainer);
      fieldContainer.appendChild(fieldValue);
...
// src/modals/BaseSelectModal.ts
var BaseSelecttModal = class extends import_obsidian37.SuggestModal {
  constructor(plugin, file, field, eF, indexedPath, lineNumber = -1, asList = false, asBlockquote = false, previousModal) {
    super(plugin.app);
    this.plugin = plugin;
    this.file = file;
  ...
  buildAddButton(container) {
    this.addButton = new import_obsidian37.ButtonComponent(container);
  ...
  buildFooterActions(footerActionsContainer) {
    footerActionsContainer.createDiv({ cls: "spacer" });
    this.buildConfirm(footerActionsContainer);
    const cancelButton = new import_obsidian37.ButtonComponent(footerActionsContainer);
    cancelButton.setIcon("cross");
    cancelButton.onClick(() => this.close());
    cancelButton.setTooltip("Cancel");
    const clearButton = new import_obsidian37.ButtonComponent(footerActionsContainer);
    ...
    if (fileClassName) {
      ...
      if (fileClass && fileClassAttribute) {
        ...
        if (Array.isArray(fileClassAttribute.options)) {
          ...
          if (valuesFile instanceof import_obsidian37.TFile && valuesFile.extension == "md") {
...
var TextInputSuggest = class {
  constructor(inputEl) {
    ...
    this.scope = new import_obsidian38.Scope();
...
var FileSuggest = class extends TextInputSuggest {
  ...
  getSuggestions(input_str) {
    ...
    all_files.forEach((file) => {
      if (file instanceof import_obsidian39.TFile && file.extension === this.extenstion && file.path.toLowerCase().contains(lower_input_str)) {
        files.push(file);
      }
    });
    return files;
...
var AbstractListBasedField = class extends FieldManager2 {
  ...
  createListNotePathContainer(container, plugin) {
    ...
    const input = new import_obsidian40.TextComponent(valuesListNotePathContainer);
    input.inputEl.addClass("full-width");
    input.inputEl.addClass("with-label");
    new FileSuggest(
      input.inputEl,
  ...
  createValueContainer(parentNode, key) {
    ...
    const input = new import_obsidian40.TextComponent(valueContainer);
    input.inputEl.addClass("full-width");
    ...
    const valueRemoveButton = new import_obsidian40.ButtonComponent(valueContainer);
    ...
    if (key != Object.keys(this.field.options)[0]) {
      const valueUpgradeButton = new import_obsidian40.ButtonComponent(valueContainer);
      (0, import_obsidian40.setIcon)(valueUpgradeButton.buttonEl, "up-chevron-glyph");
  ...
  createValuesFromDVQueryContainer(parentContainer) {
    ...
    const valuesFromDVQuery = new import_obsidian40.TextAreaComponent(valuesFromDVQueryContainer);
  ...
  createSettingContainer(container, plugin, location) {
    ...
    const sourceType = new import_obsidian40.DropdownComponent(sourceTypeContainer);
...
var SelectField = class extends AbstractListBasedField {
  ...
  createDvField(dv, p, fieldContainer, attrs = {}) {
    ...
    (0, import_obsidian41.setIcon)(dropDownButton, "down-chevron-glyph");
    const file = this.plugin.app.vault.getAbstractFileByPath(p["file"]["path"]);
...
var CycleField = class extends AbstractListBasedField {
  ...
  createSettingContainer(container, plugin, location) {
    ...
    const allowNullToggler = new import_obsidian42.ToggleComponent(allowNullValueContainer);
  ...
  createDvField(dv, p, fieldContainer, attrs = {}) {
    ...
    (0, import_obsidian42.setIcon)(cycleBtn, FieldIcon["Cycle" /* Cycle */]);
    if (!((_a = attrs == null ? void 0 : attrs.options) == null ? void 0 : _a.alwaysOn)) {
      cycleBtn.hide();
      spacer.show();
      fieldContainer.onmouseover = () => {
...
var MultiSuggestModal = class extends BaseSelecttModal {
  ...
  buildConfirm(footerActionsContainer) {
    const infoContainer = footerActionsContainer.createDiv({ cls: "info" });
    infoContainer.setText("Alt+Enter to save");
    const confirmButton = new import_obsidian43.ButtonComponent(footerActionsContainer);
    confirmButton.setIcon("checkmark");
    confirmButton.onClick(async () => {
      await this.replaceValues();
      this.close();
...
var MultiField = class extends AbstractListBasedField {
  ...
  createDvField(dv, p, fieldContainer, attrs = {}) {
    ...
    currentValues.forEach((v) => {
      ...
      (0, import_obsidian44.setIcon)(valueRemoveBtn, "cross");
      valueRemoveBtn.hide();
...
var DateModal = class extends BaseModal {
  ...
  buildPath(value) {
    ...
    while (!next.done) {
      if (next.value.groups) {
        ...
        this.pathTemplateItems[pattern] = (0, import_obsidian46.moment)(value).format(pattern);
      }
      next = tP.next();
    }
  ...
  async save() {
    ...
    if (this.plugin.app.plugins.enabledPlugins.has("nldates-obsidian")) {
      try {
        ...
        newValue = parsedDate.date ? parsedDate.moment : (0, import_obsidian46.moment)(`${this.value}`, this.format);
      } catch (error) {
        newValue = (0, import_obsidian46.moment)(`${this.value}`, this.format);
      }
    } else {
      newValue = (0, import_obsidian46.moment)(`${this.value}`, this.format);
    }
  ...
  buildInsertAsLinkButton(container) {
    const insertAsLinkBtn = new import_obsidian45.ButtonComponent(container);
  ...
  buildClearBtn(container) {
    const clearBtn = new import_obsidian45.ButtonComponent(container);
  ...
  async buildInputEl(container) {
    ...
    this.inputEl = new import_obsidian45.TextComponent(container);
    this.inputEl.inputEl.focus();
    this.inputEl.setPlaceholder(
      this.initialValue ? (0, import_obsidian46.moment)(this.initialValue, this.field.options.dateFormat).format(this.field.options.dateFormat) : ""
    );
    ...
    const calendarDisplayBtn = new import_obsidian45.ButtonComponent(container);
    calendarDisplayBtn.setIcon(FieldIcon["Date" /* Date */]);
    calendarDisplayBtn.setTooltip("open date picker");
    const shiftFromTodayBtn = new import_obsidian45.ButtonComponent(container);
    ...
    const datePicker = esm_default(datePickerContainer, {
      ...
      defaultDate: (0, import_obsidian46.moment)(Date.now()).format("YYYY-MM-DD")
    });
    datePicker.config.onChange.push((value) => {
      const newDate = (0, import_obsidian46.moment)(value.toString()).format(this.format);
...
var DateField = class extends FieldManager2 {
  ...
  createSettingContainer(container, plugin, location) {
    ...
    const dateFormatInput = new import_obsidian47.TextComponent(dateFormatContainer);
    ...
    dateExample.setText(`${(0, import_obsidian47.moment)().format(dateFormatInput.getValue())}`);
    dateFormatInput.onChange((value) => {
      this.field.options.dateFormat = value;
      dateExample.setText(`${(0, import_obsidian47.moment)().format(value)}`);
    });
    ...
    const defaultInsertAsLink = new import_obsidian47.ToggleComponent(defaultInsertAsLinkContainer);
    ...
    const dateLinkPathInput = new import_obsidian47.TextComponent(dateLinkPathContainer);
    ...
    const dateShiftInterval = new import_obsidian47.TextComponent(dateShiftIntervalContainer);
    ...
    const nextShiftIntervalField = new import_obsidian47.DropdownComponent(nextShiftIntervalFieldContainer);
  ...
  async getMomentDate(file, indexedPath) {
    ...
    return (0, import_obsidian47.moment)(_dateText, dateFormat);
  }
  async getNewDateValue(currentShift, file, indexedPath) {
    ...
    return _newDate || (0, import_obsidian47.moment)().format(dateFormat);
  }
  ...
  createDvField(dv, p, fieldContainer, attrs = {}) {
    ...
    (0, import_obsidian47.setIcon)(dateBtn, FieldIcon["Date" /* Date */]);
    const spacer = fieldContainer.createDiv({ cls: "spacer-1" });
    this.shiftBtn = fieldContainer.createEl("button");
    (0, import_obsidian47.setIcon)(this.shiftBtn, "skip-forward");
  ...
  displayValue(container, file, value, onClicked) {
    ...
    if (dateLink == null ? void 0 : dateLink.path) {
      ...
      const date = (0, import_obsidian47.moment)(value, dateFormat);
      if (date.isValid()) {
  ...
  validateValue(value) {
    ...
    if (!value) {
      ...
      if (typeof value == "string") {
        return (0, import_obsidian47.moment)(
  ...
  async shiftDuration(file) {
    ...
    if (import_obsidian47.moment.isDuration(import_obsidian47.moment.duration(nextShiftNumber, nextShiftPeriod))) {
...
var MultiFileModal = class extends import_obsidian48.FuzzySuggestModal {
  constructor(plugin, file, field, eF, indexedPath, lineNumber = -1, asList = false, asBlockquote = false, previousModal) {
    var _a;
    super(plugin.app);
    this.plugin = plugin;
    ...
    if (initialOptions) {
      if (Array.isArray(initialOptions)) {
        initialOptions.map((item) => {
          ...
          if (link) {
            const file2 = this.plugin.app.vault.getAbstractFileByPath(link.path);
            if (file2 instanceof import_obsidian48.TFile && !this.selectedFiles.map((_f) => _f.path).includes(file2.path))
        ...
        links.forEach((_link) => {
          ...
          if (link) {
            const file2 = this.plugin.app.vault.getAbstractFileByPath(link.path);
            if (file2 instanceof import_obsidian48.TFile && !this.selectedFiles.map((_f) => _f.path).includes(link.path))
              this.selectedFiles.push(file2);
          }
        });
      }
    ...
    const infoContainer = buttonContainer.createDiv({ cls: "info" });
    infoContainer.setText("Alt+Enter to save");
    const confirmButton = new import_obsidian48.ButtonComponent(buttonContainer);
    confirmButton.setIcon("checkmark");
    confirmButton.onClick(async () => {
      await this.replaceValues();
      this.close();
    });
    const cancelButton = new import_obsidian48.ButtonComponent(buttonContainer);
    cancelButton.setIcon("cross");
    cancelButton.onClick(() => {
      this.close();
    });
    const clearButton = new import_obsidian48.ButtonComponent(buttonContainer);
  ...
  renderSuggestion(value, el) {
    ...
    if (this.selectedFiles.some((file) => file.path === value.item.path)) {
      el.addClass("value-checked");
      const iconContainer = el.createDiv({ cls: "icon-container" });
      (0, import_obsidian48.setIcon)(iconContainer, "check-circle");
    }
  }
  renderSelected() {
    const chooser = this.chooser;
...
async function updateLookups(plugin, forceUpdateOne, forceUpdateAll = false) {
  ...
  await Promise.all([...f.fileLookupFiles.keys()].map(async (lookupFileId) => {
    ...
    if (!file || !(file instanceof import_obsidian49.TFile))
      return;
...
var LookupField = class extends FieldManager2 {
  ...
  createLookupContainer(container) {
    ...
    const autoUpdate = new import_obsidian50.ToggleComponent(autoUpdateContainer);
    ...
    const dvQueryStringContainer = dvQueryStringTopContainer.createDiv({ cls: "field-container" });
    const dvQueryString = new import_obsidian50.TextAreaComponent(dvQueryStringContainer);
    ...
    const targetFieldName = new import_obsidian50.TextComponent(targetFieldContainer);
    ...
    const outputTypeList = new import_obsidian50.DropdownComponent(outputTypeContainer);
    ...
    const builtinSummarizeFunctionList = new import_obsidian50.DropdownComponent(builtinSummarizeFunctionContainer);
    ...
    const summarizedFieldName = new import_obsidian50.TextComponent(summarizedFieldNameContainer);
    ...
    const outputRenderingFunction = new import_obsidian50.TextAreaComponent(outputRenderingFunctionContainer);
    ...
    const outputSummarizingFunction = new import_obsidian50.TextAreaComponent(outputSummarizingFunctionContainer);
...
async function updateFormulas(plugin, forceUpdateOne, forceUpdateAll = false) {
  ...
  await Promise.all([...fileFormulasFields].map(async ([id, field]) => {
    ...
    if (!_file || !(_file instanceof import_obsidian51.TFile))
      return;
...
var FormulaField = class extends FieldManager2 {
  ...
  createFormulaContainer(container) {
    ...
    autoUpdateContainer.createEl("span", { text: "Auto update this field ", cls: "label" });
    autoUpdateContainer.createDiv({ cls: "spacer" });
    const autoUpdate = new import_obsidian52.ToggleComponent(autoUpdateContainer);
    autoUpdateTopContainer.createEl("span", { text: "This could lead to latencies depending on the queries", cls: "sub-text warning" });
    if (this.field.options.autoUpdate === void 0)
      this.field.options.autoUpdate = false;
    autoUpdate.setValue(this.field.options.autoUpdate);
    ...
    const formula = new import_obsidian52.TextAreaComponent(formulaContainer);
  ...
  constructor(plugin, field, type) {
    ...
    this.buildColorsContainer = (container, colorList, label) => {
      ...
      const toggleStandardColorButton = (container2, color) => {
        if (colorList.includes(color)) {
          ...
          (0, import_obsidian53.setIcon)(container2, "cross");
        } else {
          container2.removeClass("active");
          (0, import_obsidian53.setIcon)(container2, "plus");
        }
        ;
      };
      ...
      const toggleAltColors = () => {
        ...
        altGroupColors.forEach((color) => {
          ...
          (0, import_obsidian53.setIcon)(colorContainer, "cross");
          colorContainer.onclick = () => {
      ...
      const altColorPicker = new import_obsidian53.ColorComponent(altColorPickerContainer);
    ...
    this.buildEdgeSideContainer = (container, edgeList, label) => {
      ...
      this.sides.forEach(([side, iconName]) => {
        ...
        (0, import_obsidian53.setIcon)(sideIconContainer, iconName);
        const sideTogglerContainer = new import_obsidian53.ToggleComponent(edgeSideContainer);
    ...
    this.buildLabelsContainer = (container, labels, title) => {
      ...
      labels.forEach((label) => {
        ...
        new import_obsidian53.ButtonComponent(labelContainer).setIcon("x-circle").setClass("item-remove").onClick(() => {
          labels.remove(label);
    ...
    this.buildNewLabelContainer = (currentLabelsContainer, currentLabelsTitle, newLabelContainer, labels, title) => {
      ...
      const labelInput = new import_obsidian53.TextComponent(newLabelContainer);
      const labelValidate = new import_obsidian53.ButtonComponent(newLabelContainer);
    ...
    this.createCanvasPathContainer = (container) => {
      ...
      const canvasPathInput = new import_obsidian53.TextComponent(container);
    ...
    this.createDirectionContainer = (container, title) => {
      ...
      container.createDiv({ cls: "spacer" });
      const directionSelection = new import_obsidian53.DropdownComponent(container);
      [
        ["incoming", "Incoming"],
        ["outgoing", "Outgoing"],
    ...
    this.createDvQueryContainer = (container, title) => {
      ...
      const filesFromDVQuery = new import_obsidian53.TextAreaComponent(filesFromDVQueryContainer);
...
var RawObjectField = class extends FieldManager2 {
  ...
  createDvField(dv, p, fieldContainer, attrs = {}) {
    ...
    if ((_a = attrs.options) == null ? void 0 : _a.alwaysOn)
      spacer.hide();
    (0, import_obsidian54.setIcon)(editBtn, FieldIcon["Input" /* Input */]);
    if (!((_b = attrs == null ? void 0 : attrs.options) == null ? void 0 : _b.alwaysOn)) {
      editBtn.hide();
      spacer.show();
      fieldContainer.onmouseover = () => {
...
var ObjectListModal = class extends BaseSuggestModal {
  ...
  buildAddButton(container) {
    ...
    const addButton = new import_obsidian55.ButtonComponent(container);
  ...
  renderSuggestion(item, el) {
    ...
    (0, import_obsidian55.setIcon)(removeContainer, "trash");
...
var ObjectListField = class extends FieldManager2 {
  ...
  */
  ...
  createDvField(dv, p, fieldContainer, attrs) {
    ...
    fieldContainer.appendChild(fieldValue);
    const editBtn = fieldContainer.createEl("button");
    (0, import_obsidian56.setIcon)(editBtn, FieldIcon[this.field.type]);
    editBtn.onclick = async () => {
      const file = this.plugin.app.vault.getAbstractFileByPath(p["file"]["path"]);
      const _eF = file instanceof import_obsidian56.TFile && file.extension == "md" && await Note.getExistingFieldForIndexedPath(this.plugin, file, this.field.id);
      if (_eF)
        this.createAndOpenFieldModal(file, this.field.name, _eF, _eF.indexedPath);
    };
  }
...
var FieldOptions = class {
  ...
  addOption(icon, onclick, tooltip, className, file, indexedPath, plugin) {
    ...
    const fieldOption = new import_obsidian57.ButtonComponent(fieldOptionContainer);
...
var FieldsModal = class extends import_obsidian57.Modal {
  ...
  buildNavigation() {
    ...
    const backBtn = new import_obsidian57.ButtonComponent(backBtnWrapper);
  ...
  buildFieldContainer(container, field, value, indexedPath) {
    ...
    const fieldSettingBtn = new import_obsidian57.ButtonComponent(fieldSettingContainer);
    ...
    if (this.existingFields.map((_f) => _f.field.id).includes(field.id)) {
      ...
      const fieldBtn = new import_obsidian57.ButtonComponent(fieldBtnContainer);
      ...
      ancestors.forEach(async (fileClassName, i) => {
        ...
        if (_fileClass) {
          ...
          if (await _fileClass.missingFieldsForFileClass(this.file)) {
            const fileClassInsertMissingFieldsInFrontmatterBtn = new import_obsidian57.ButtonComponent(fileClassOptionsContainer);
            ...
            const fileClassInsertMissingFieldsBtn = new import_obsidian57.ButtonComponent(fileClassOptionsContainer);
          ...
          const fileClassAddAttributeBtn = new import_obsidian57.ButtonComponent(fileClassOptionsContainer);
  ...
  buildInsertMissingFieldsBtn() {
    ...
    const insertMissingFieldsInFrontmatterBtn = new import_obsidian57.ButtonComponent(insertMissingFieldsContainer);
    ...
    const insertMissingFieldsBtn = new import_obsidian57.ButtonComponent(insertMissingFieldsContainer);
  ...
  buildInsertNewItem(field, indexedPath) {
    ...
    const insertNewItemBtn = new import_obsidian57.ButtonComponent(insertNewItemContainer);
...
var NoteFieldsComponent = class extends import_obsidian57.Component {
...
function addFileClassAttributeOptions(plugin) {
  ...
  plugin.addCommand({
    ...
    checkCallback: (checking) => {
      const view = plugin.app.workspace.getActiveViewOfType(import_obsidian58.MarkdownView);
    ...
    checkCallback: (checking) => {
      ...
      if (inFileClass) {
        try {
          ...
          new import_obsidian58.Notice("plugin is not a valid fileClass");
        }
      }
    }
  });
            ...
            case "source":
              {
                ...
                if (node)
                  ...
                  new import_obsidian58.Notice("No field with definition at this position", 2e3);
              }
              break;
            case "preview": {
...
function addOpenFieldsModalCommand(plugin) {
  ...
  plugin.addCommand({
    ...
    checkCallback: (checking) => {
      ...
      if (inFile) {
        const file = view.file;
        if (inFile && file instanceof import_obsidian58.TFile && file.extension === "md") {
...
var ContextMenu = class extends import_obsidian59.Component {
  constructor(plugin) {
    super();
    this.plugin = plugin;
  ...
  buildOptions(file, menu) {
    const classFilesPath = this.plugin.settings.classFilesPath;
    if (file instanceof import_obsidian59.TFile && file.extension === "md") {
      if (!import_obsidian59.Platform.isMobile && (0, import_obsidian59.requireApiVersion)("0.16.0")) {
...
function setLinkMetadataFormButton(plugin, link, destPath, viewTypeName, fileClassName) {
  ...
  if (classFilessPath && fileClass) {
    ...
    if (!(el == null ? void 0 : el.hasClass("fileclass-icon"))) {
      ...
      if (metadataMenuBtn) {
        (0, import_obsidian60.setIcon)(metadataMenuBtn, icon);
    ...
    if (fileClass2) {
      ...
      if (!(el == null ? void 0 : el.hasClass("fileclass-icon"))) {
        ...
        setStatus(metadataMenuBtn);
        if (metadataMenuBtn) {
          (0, import_obsidian60.setIcon)(metadataMenuBtn, icon || plugin.settings.buttonIcon);
    ...
    default:
      {
        ...
        const dest = linkName && app2.metadataCache.getFirstLinkpathDest((0, import_obsidian60.getLinkpath)(linkName), sourceName);
        if (dest) {
...
function updateVisibleLinks(app2, plugin) {
  ...
  app2.workspace.iterateRootLeaves((leaf) => {
    var _a;
    if (leaf.view instanceof import_obsidian60.MarkdownView && leaf.view.file) {
    ...
    toDOM() {
      ...
      if (this.fileClassName) {
        ...
        if (classFilesPath && this.destName.includes(classFilesPath)) {
          ...
          if (fileClass) {
            (0, import_obsidian61.setIcon)(metadataMenuBtn, icon || settings.buttonIcon);
            metadataMenuBtn.onclick = (event) => {
              const fileClassViewManager = new FileClassViewManager(plugin, fileClass);
              plugin.addChild(fileClassViewManager);
              fileClassViewManager.build();
  ...
  const viewPlugin = import_view3.ViewPlugin.fromClass(
    class {
      ...
      buildDecorations(view) {
        ...
        const mdView = view.state.field(import_obsidian61.editorInfoField);
        let lastAttributes = {};
...
// src/components/ExtraButton.ts
var ExtraButton = class extends import_obsidian62.Component {
  constructor(plugin) {
    super();
    this.plugin = plugin;
    this.modalObservers = [];
  ...
  onload() {
    ...
    this.registerEvent(this.plugin.app.metadataCache.on("changed", (0, import_obsidian62.debounce)(this.updateLinks, 100, true)));
    this.registerEvent(this.plugin.app.workspace.on("metadata-menu:indexed", (0, import_obsidian62.debounce)(this.updateLinks, 100, true)));
    this.registerEvent(this.plugin.app.workspace.on("layout-change", (0, import_obsidian62.debounce)(this.updateLinks, 10, true)));
    ...
    this.registerEvent(this.plugin.app.internalPlugins.getPluginById("bookmarks").instance.on("changed", (0, import_obsidian62.debounce)(this.updateLinks, 100, true)));
  }
  initViewObservers() {
...
var FileClassQuery = class {
  ...
  getResults(api) {
    try {
      ...
      new import_obsidian63.Notice(` for <${this.name}>. Check your settings`);
      return [];
    }
  }
  matchFile(file) {
  ...
  canvases.forEach(async (canvas) => {
    ...
    if (rawContent) {
      try {
        ...
        new import_obsidian64.Notice(`Couldn't read ${canvas.path}`);
      }
    }
    ...
    currentFiles.forEach(async ({ cumulatedLinksFields, cumulatedGroupsFields, cumulatedGroupsLinksFields }, filePath) => {
      const file = plugin.app.vault.getAbstractFileByPath(filePath);
      if (file && file instanceof import_obsidian64.TFile) {
    ...
    previousFilesPaths.filter((f2) => !currentFilesPaths.includes(f2)).forEach(async (filePath) => {
      ...
      if (targetFile && targetFile instanceof import_obsidian64.TFile) {
        const payload = [];
  ...
  for (const file of files) {
    ...
    if (index.classFilesPath && file.path.startsWith(this.classFilesPath)) {
      ...
      await Promise.all(canvasFields.map(async (field) => {
        ...
        if (canvasFile instanceof import_obsidian64.TFile && canvasFile.extension === "canvas") {
...
var FieldIndexBuilder = class extends import_obsidian65.Component {
  constructor(plugin) {
    super();
    this.plugin = plugin;
    this.changedFiles = [];
  ...
  async onload() {
    ...
    this.registerEvent(
      this.plugin.app.vault.on("modify", async (file) => {
        if (file instanceof import_obsidian66.TFile) {
    ...
    canvases.forEach(async (canvas) => {
      ...
      if (rawContent) {
        try {
          ...
          DEBUG && console.log(error);
          new import_obsidian66.Notice(`Couldn't read ${canvas.path}`);
        }
      }
...
var IndexStatus = class extends import_obsidian67.Component {
  constructor(plugin) {
    super();
    this.plugin = plugin;
  ...
  onload() {
    ...
    this.statusBtn = new import_obsidian67.ButtonComponent(container);
  ...
  checkForUpdate(view) {
    if (view && view instanceof import_obsidian67.FileView && view.file) {
...
function fieldModifier(plugin, dv, p, fieldName, attrs) {
  ...
  if (p[fieldName] === void 0) {
    if (!((_a = attrs == null ? void 0 : attrs.options) == null ? void 0 : _a.showAddField)) {
      ...
      (0, import_obsidian68.setIcon)(addFieldBtn, "log-in");
      ...
      (0, import_obsidian68.setIcon)(addInFrontmatterFieldBtn, "align-vertical-space-around");
...
async function fileFields(plugin, fileOrfilePath) {
  let file;
  if (fileOrfilePath instanceof import_obsidian69.TFile) {
    file = fileOrfilePath;
  } else {
    const _file = plugin.app.vault.getAbstractFileByPath(fileOrfilePath);
    if (_file instanceof import_obsidian69.TFile && _file.extension == "md") {
      file = _file;
    } else {
      throw Error("path doesn't correspond to a proper file");
    }
...
var FolderSuggest = class extends TextInputSuggest {
  ...
  getSuggestions(inputStr) {
    ...
    abstractFiles.forEach((folder) => {
      if (folder instanceof import_obsidian71.TFolder && folder.path.toLowerCase().contains(lowerCaseInputStr)) {
        folders.push(folder);
      }
    });
    return folders;
...
var FileClassQuerySetting = class extends import_obsidian72.Setting {
...
// src/settings/FileClassQuerySettingModal.ts
var FileClassQuerySettingsModal = class extends import_obsidian73.Modal {
  ...
  createnameInputContainer(container) {
    ...
    const input = new import_obsidian73.TextComponent(container);
    input.inputEl.addClass("with-label");
    input.inputEl.addClass("full-width");
  ...
  createFileClassSelectorContainer(container) {
    ...
    container.createDiv({ cls: "spacer" });
    const select = new import_obsidian73.DropdownComponent(container);
    const classFilesPath = this.plugin.settings.classFilesPath;
  ...
  createQueryInputContainer(container) {
    ...
    const queryStringInput = new import_obsidian73.TextAreaComponent(queryStringInputContainer);
  ...
  createSaveButton(container) {
    const b = new import_obsidian73.ButtonComponent(container);
    b.setTooltip("Save");
    b.setIcon("checkmark");
    b.onClick(async () => {
      var _a;
...
// src/settings/MetadataMenuSettingTab.ts
var SettingTextWithButtonComponent = class extends import_obsidian74.Setting {
  constructor(plugin, containerEl, name, description, placeholder, currentValues, normalizeValue) {
    ...
    this.newValues = [];
    const saveButton = new import_obsidian74.ButtonComponent(this.containerEl);
...
var ButtonDisplaySetting = class extends import_obsidian74.Setting {
...
var MetadataMenuSettingTab = class extends import_obsidian74.PluginSettingTab {
  constructor(plugin) {
  ...
  createSettingGroup(title, subTitle, withButton = false) {
    ...
    if (withButton) {
      ...
      const settingsContainerShowButton = new import_obsidian74.ButtonComponent(settingsContainerShowButtonContainer);
  ...
  display() {
    ...
    new import_obsidian74.Setting(globalSettings).setName("Scope").setDesc("Index fields in frontmatter only or in the whole note (if you use dataview inline fields). Indexing full notes could cause some latencies in vaults with large files").addDropdown((cb) => {
    ...
    new import_obsidian74.Setting(globalSettings).setName("Display field options in context menu").setDesc("Choose to show or hide fields options in the context menu of a link or a file").addToggle((toggle) => {
    ...
    const enableAutoComplete = new import_obsidian74.Setting(globalSettings).setName("Autocomplete").setDesc("Activate autocomplete fields").addToggle((cb) => {
    ...
    const showIndexingStatus = new import_obsidian74.Setting(globalSettings).setName("Fields Indexing Status").setDesc("Show fields indexing status icon in status toolbar").addToggle((cb) => {
    ...
    const frontmatterListDisplay = new import_obsidian74.Setting(globalSettings).setName("Frontmatter list display").setDesc("Choose wether lists should be displayed as arrays or indented lists in frontmatter").addDropdown((cb) => {
    ...
    new import_obsidian74.Setting(globalSettings).setName("First day of week").setDesc("For date fields, which day the date picker's week should start with").addDropdown((cb) => {
      for (let i = 0; i < 2; i++) {
        cb.addOption(i.toString(), (0, import_obsidian74.moment)().day(i).format("dddd"));
      }
    ...
    new import_obsidian74.Setting(presetFieldsSettings).setName("Add New Field Setting").setDesc("Add a new Frontmatter property for which you want preset options.").addButton((button) => {
    ...
    const fileClassesFolderSaveButton = new import_obsidian74.ButtonComponent(classFilesSettings);
    ...
    const path = new import_obsidian74.Setting(classFilesSettings).setName("class Files path").setDesc("Path to the files containing the authorized fields for a type of note").addSearch((cfs) => {
    ...
    const aliasSaveButton = new import_obsidian74.ButtonComponent(classFilesSettings);
    ...
    const alias = new import_obsidian74.Setting(classFilesSettings).setName("fileClass field alias").setDesc("Choose another name for fileClass field in frontmatter (example: Category, type, ...").addText((text) => {
    ...
    const global = new import_obsidian74.Setting(classFilesSettings).setName("global fileClass").setDesc("Choose one fileClass to be applicable to all files (even it is not present as a fileClass attribute in their frontmatter). This will override the preset Fields defined above").addSearch((cfs) => {
      new FileSuggest(
        cfs.inputEl,
        this.plugin,
    ...
    const defaultIconSave = new import_obsidian74.ButtonComponent(classFilesSettings);
    ...
    const defaultIconSetting = new import_obsidian74.Setting(classFilesSettings).setName("Default Icon").setDesc("Choose a default icon for fileclasses from lucide.dev library").addText((cb) => {
      cb.setValue(this.plugin.settings.fileClassIcon || DEFAULT_SETTINGS.fileClassIcon).onChange((value) => {
        this.newIcon = value;
        (0, import_obsidian74.setIcon)(iconManagerContainer, value);
        defaultIconSave.setCta();
      });
    });
    (0, import_obsidian74.setIcon)(iconManagerContainer, this.plugin.settings.fileClassIcon || DEFAULT_SETTINGS.fileClassIcon);
    ...
    const rowPerPageSaveButton = new import_obsidian74.ButtonComponent(classFilesSettings);
    ...
    const maxRows = new import_obsidian74.Setting(classFilesSettings).setName("Result per page").setDesc("Number of result per page in table view").addText((text) => {
    ...
    const chooseFileClassAtFileCreation = new import_obsidian74.Setting(classFilesSettings).setName("Add a fileclass after create").setDesc("Select a fileclass at file creation to be added to the file").addToggle((cb) => {
    ...
    const showFileClassSelectInModal = new import_obsidian74.Setting(classFilesSettings).setName("Fileclass Select").setDesc("Show fileclass select option in note fields modals").addToggle((cb) => {
    ...
    new import_obsidian74.Setting(metadataMenuBtnSettings).setName("Metadata Menu button icon").setDesc("name of the default icon when not defined in fileClass").addText((text) => {
    ...
    new import_obsidian74.Setting(queryFileClassSettings).setName("Add New Query for fileClass").setDesc("Add a new query and a FileClass that will apply to files matching this query.").addButton((button) => {
...
var listPrefix = "  - ";
var ValueSuggest = class extends import_obsidian75.EditorSuggest {
  constructor(plugin) {
    super(plugin.app);
  ...
  async selectSuggestion(suggestion, event) {
    var _a, _b, _c, _d, _e;
    const activeView = this.plugin.app.workspace.getActiveViewOfType(import_obsidian75.MarkdownView);
    if (!activeView) {
      return;
    }
    ;
    ...
    if (this.inFrontmatter && file) {
      try {
        ...
        let parsedField2 = (0, import_obsidian75.parseYaml)(serializedField);
  ...
  - ${clean(value)}`).join("")}`;
        ...
        new import_obsidian75.Notice("Frontmatter wrongly formatted", 2e3);
        this.close();
        return;
      }
...
var updateProps = async (plugin, view) => {
  if (!(view instanceof import_obsidian76.MarkdownView) || !(view.file instanceof import_obsidian76.TFile) || view.file === void 0)
    return;
  const file = view.file;
  ...
  view.metadataEditor.rendered.forEach((item) => {
    ...
    if (plugin.settings.enableProperties) {
      ...
      const btn = new import_obsidian76.ButtonComponent(btnContainer);
  ...
  fileClasses == null ? void 0 : fileClasses.forEach((fileClass) => {
    const addFieldButton = new import_obsidian76.ButtonComponent(fileClassButtonsContainer);
...
async function updatePropertiesSection(plugin) {
  ...
  const currentView = plugin.app.workspace.getActiveViewOfType(import_obsidian76.MarkdownView);
...
var FileClassFolderButton = class extends import_obsidian77.Component {
  constructor(plugin) {
    super();
    this.plugin = plugin;
    this.addButton();
  ...
  addButton() {
    ...
    if (!container.findAll(".fileClass-add-button").length) {
      ...
      (0, import_obsidian77.setIcon)(addBtn, "plus-circle");
      addBtn.onclick = (e) => {
        e.preventDefault();
...
var StoreManager = class extends import_obsidian78.Component {
...
var IndexDatabase = class extends import_obsidian79.Component {
  constructor(plugin) {
    super();
    this.plugin = plugin;
    this.init();
...
var FileClassCodeBlockView = class {
  constructor(plugin, tableId, fileClass, paginationContainer, tableContainer, selectedView, ctx) {
    this.plugin = plugin;
    this.tableId = tableId;
    this.fileClass = fileClass;
  ...
  getViewConfig() {
    ...
    const defaultConfig = {
      ...
      columns: [{
        ...
        hidden: false,
        position: 0
      }].concat(((_a = this.plugin.fieldIndex.fileClassesFields.get(this.fileClass.name)) == null ? void 0 : _a.filter((f) => f.isRoot()).map((f, index) => {
        return {
          name: f.name,
          hidden: false,
          position: index + 1
    ...
    return ((_b = options2.savedViews) == null ? void 0 : _b.find((view) => view.name === this.selectedView)) || defaultConfig;
  }
  update(maxRows, sliceStart = 0) {
    this.fileClassDataviewTable = new FileClassDataviewTable(
      this.plugin,
...
// src/components/FileClassCodeBlockManager.ts
var FileClassCodeBlockManager = class extends import_obsidian80.MarkdownRenderChild {
  ...
  build() {
    ...
    container.createDiv();
    try {
      const content = (0, import_obsidian80.parseYaml)(source);
...
// main.ts
var MetadataMenu = class extends import_obsidian81.Plugin {
  constructor() {
    super(...arguments);
    this.presetFields = [];
    this.initialFileClassQueries = [];
    this.launched = false;
    this.codeBlockManagers = [];
  }
  async onload() {
...
------------------------------------------`,
    ...
    this.registerEvent(
      this.app.vault.on("create", (file) => {
        if (!this.fieldIndex.fileClassesName.size)
          return;
        if (file instanceof import_obsidian81.TFile && this.settings.chooseFileClassAtFileCreation) {
    ...
    this.registerEvent(
      this.app.workspace.on("metadata-menu:indexed", () => {
        ...
        const currentView = this.app.workspace.getActiveViewOfType(import_obsidian81.MarkdownView);
        if (currentView)
